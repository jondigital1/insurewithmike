...
export const QUESTIONNAIRE: Section[] = [
  {
    ...
    // No code question. The code is generated by the system when the form is
    // submitted and shown on the thank you page for the client to pass to
    // their agent, whose CRM holds the join between code and person. Asking
    // the client to carry a code IN was the old flow; it put a free field on
    // the form that would accept anything, and it made the broker issue codes
    // before anyone had answered anything.
    //
    // The blurb deliberately does not mention the code. The hero's "How this
    // works" owns that promise, one screen above; repeating it here made the
    // page say the same sentence twice within a scroll.
...

  {
    ...
    questions: [
      ...
      {
        id: "current_insurer_feeling",
        kind: "choice",
        label: "How do you feel about them?",
        options: [
          { value: "keep", label: "Happy. I would rather stay if the numbers work" },
          { value: "neutral", label: "No strong feelings either way" },
          { value: "leave", label: "Unhappy. I want to look elsewhere" },
        ],
        required: false,
        // Chained off the carrier answer rather than the coverage fork, so
        // "how do you feel about them?" can only appear once "them" has a
        // name. A client who answered "someone else" or "not sure" never
        // sees it, and the uninsured path hides it transitively because the
        // carrier question itself is hidden there. Aetna is excluded because
        // that client is moving regardless, so the feeling changes nothing
        // the form can act on.
        showIf: {
          question: "current_insurer",
          equals: ["horizon", "amerihealth", "oscar", "unitedhealthcare", "ambetter"],
        },
        routing: "intake",
        rationale:
          "Unlike asking what someone will prioritise, which people guess at, this asks what they already experienced, which they know. Carrier loyalty and carrier aversion are both among the likeliest reasons an agent overrides a ranking, and the answer changes what the shortlist leads with: happy anchors a staying-put option, unhappy suppresses it and tells the agent to open by asking what went wrong. It never hides the carrier's plans. If the disliked carrier is $4,000 cheaper, that belongs in the conversation, priced.",
      },
      {
...

  {
    ...
    questions: [
      ...
      {
        // A flag rather than an input on purpose: dental prices from the ages
        // already collected, and vision cannot be priced from public data at
        // all. The answer changes what the agent brings to the meeting, never
        // what the engine computes, so it earns no follow-up questions.
        id: "dental_vision_interest",
        kind: "multichoice",
        label: "Are you interested in dental or vision coverage as well?",
        options: [
        ...
        routing: "flag",
        rationale:
          "Saying yes adds no extra questions. Dental plans here are priced from ages alone, children's vision is already part of every marketplace plan, and adult vision is something your agent can walk through with you.",
      },
      {
        id: "anything_else",
        kind: "longtext",
