...
${TOKENS}
  ...
  * { box-sizing: border-box; }
  ...
  @media print {
    ...
    .topbar .btn, .topbar .viewtoggle, .notice { display: none; }
...
(function () {
  ...
  function render(result, submittedAt) {
    ...
    // ---- everything else
    html += '<section class="card"><h2>Every plan available to them</h2><div class="scroll"><table class="plans">';
    // No worst case column. The table gets printed and read across the desk
    // alongside the cards, so leaving it here would have undone taking it off
    // them. The figure is still computed and still ranked on; it is out of the
    // client's eyeline, not out of the maths.
    html += '<thead><tr><th>Carrier</th><th>Plan</th><th>Metal</th><th class="n">Premium/mo</th><th class="n">Out of pocket</th><th class="n">Expected total</th></tr></thead><tbody>';
    for (const e of result.allEligible) {
      html += '<tr><td class="carrier">' + esc(e.plan.issuerName.split(" ")[0]) + '</td><td class="planname">' + esc(e.plan.marketingName) + '</td><td>' + esc(e.plan.metalLevel) + '</td>' +
        '<td class="n">' + usd(e.cost.annualPremiumNet / 12) + '</td>' +
        '<td class="n">' + usd(e.cost.estimatedOutOfPocket) + '</td>' +
        '<td class="n">' + usd(e.cost.estimatedAnnualTotal) + '</td></tr>';
    }
    html += '</tbody></table></div>';
